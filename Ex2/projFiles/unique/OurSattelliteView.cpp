...
OurSatelliteView::OurSatelliteView(vector<vector<array<shared_ptr<matrixObject>, 3>>> gBoard, int numOfRows, int numOfCols, size_t tankX, size_t tankY): gameBoard(gBoard), numOfRows(numOfRows), numOfCols(numOfCols), tankX(tankX), tankY(tankY) {
...
char OurSatelliteView::getObjectAt(size_t x, size_t y) const {
    ...
    if(gameBoard[y][x][1] != nullptr){
        if(gameBoard[y][x][1]->getType() == P1T){
            return '1'; // Player 1 tank
        }
        else if(gameBoard[y][x][1]->getType() == P2T){
            return '2'; // Player 2 tank
        }
        return '*'; // Bullet
    }
    if(gameBoard[y][x][0] != nullptr){
        if(gameBoard[y][x][0]->getType() == W){
            return '#'; // Wall
        }
        else if(gameBoard[y][x][0]->getType() == M){
            return '@'; // Mine
        }
    }
    return ' ' ;// Empty space
